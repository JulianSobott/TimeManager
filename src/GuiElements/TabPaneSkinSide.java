...
public class TabPaneSkinSide extends SkinBase<TabWindow> {
     ...
     * nodes into the Control list, as
     * well as the necessary input mappings for handling key, mouse, etc events.
     *
     * @param control The control that this skin should be installed onto.
     */
    public TabPaneSkinSide(TabWindow control) {
        super(control);
    ...
    @Override protected void layoutChildren(final double x, final double y,
        ...
        for (int i = 0, max = tabContentRegions.size(); i < max; i++) {
            ...
            tabHeaderArea.widthProperty().addListener(l -> {
                double headerW = tabHeaderArea.getWidth();
                double contentW = w - headerW;
                tabContent.relocate(headerW, y);
                tabContent.resize(contentW, contentHeight);
                if (tabContent.getClip() != null) {
                    ((Rectangle)tabContent.getClip()).setWidth(contentW);
                    ((Rectangle)tabContent.getClip()).setHeight(contentHeight);
                }
            });
        }
...
//        tabHeaderAreaClipRect.setWidth(10);
    ...
    private static class StyleableProperties {
        ...
        private final static CssMetaData<TabPane,TabAnimation> OPEN_TAB_ANIMATION =
                new CssMetaData<TabPane, TabPaneSkinSide.TabAnimation>("-fx-open-tab-animation",
                    ...
                    @Override public StyleableProperty<TabAnimation> getStyleableProperty(TabPane node) {
                        TabPaneSkinSide skin = (TabPaneSkinSide) node.getSkin();
        ...
        private final static CssMetaData<TabPane,TabAnimation> CLOSE_TAB_ANIMATION =
                new CssMetaData<TabPane, TabPaneSkinSide.TabAnimation>("-fx-close-tab-animation",
                        new EnumConverter<TabAnimation>(TabAnimation.class), TabAnimation.GROW) {
    ...
    class TabHeaderArea extends StackPane {
        ...
        public TabHeaderArea() {
            ...
            menu = new StackPane();
            menu.getChildren().add(menuView);
            menu.setPrefSize(menuImage.getWidth(), menuImage.getHeight());
...
//            setupReordering(headersRegion);
            ...
            getChildren().addAll(headerBackground, menu, headersRegion, controlButtons);
        ...
        private boolean tabsFit() {
            double headerPrefHeight = snapSize(headersRegion.prefHeight(-1));
            double visibleHeight = headerPrefHeight;
            return visibleHeight < getHeight();
        }
        ...
        @Override protected double computePrefWidth(double height) {
            return snapSizeX(headersRegion.prefWidth(height));
        }
        ...
        public TabHeaderSkin(final Tab tab) {
            ...
            inner = new StackPane() {
                @Override protected void layoutChildren() {
                    ...
                    final double childrenWidth = labelAreaWidth ;
                    final double childrenHeight = labelHeight;

                    if (childrenWidth > maxWidth && maxWidth != Double.MAX_VALUE) {
                        labelAreaWidth = maxWidth;
                        labelWidth = maxWidth;
                    } else if (childrenWidth < minWidth) {
                        labelAreaWidth = minWidth;
                    }
    ...
    static class TabMenuItem extends RadioMenuItem {
        ...
        public TabMenuItem(final Tab tab) {
            super(tab.getText(), TabPaneSkinSide.clone(tab.getGraphic()));
            this.tab = tab;
            setDisable(tab.isDisable());
